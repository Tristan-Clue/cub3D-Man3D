...
/*   Updated: 2026/02/11 17:14:40 by kchiang          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int argc, char **argv)
{
	...
	data = (t_data){0};
	parse_mapdata(&data.mapdata, argv[1]);
	...
	return (free(data.mlx), delete_mapdata(&data.mapdata), EXIT_SUCCESS);
}
...
static int	init_mlx(t_data *data)
{
	data->mlx = mlx_init();
	if (!data->mlx)
		return (delete_mapdata(&data->mapdata), FAILURE);
	data->window = mlx_new_window(data->mlx, FRAME_WIDTH, FRAME_HEIGHT, TITLE);
	if (!data->window)
		return (delete_mapdata(&data->mapdata),
	...
	if (!data->img.img_ptr)
	{
		...
		mlx_destroy_display(data->mlx);
		return (free(data->mlx), delete_mapdata(&data->mapdata), FAILURE);
	}
	...
	if (!data->img.px)
	{
		...
		mlx_destroy_window(data->mlx, data->window);
		mlx_destroy_display(data->mlx);
		(void)(free(data->mlx), delete_mapdata(&data->mapdata));
		error_exit("mlx_get_data_addr failure");
	}
	return (SUCCESS);
}

static void	delete_mapdata(t_mapdata *mapdata)
{
}
